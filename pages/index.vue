...
<div class="w-full flex-col flex py-20 items-center bg-gradient-to-b from-topGradient">
    <img :src="index.story.content.logo" alt="" class="w-20">
    <img :src="index.story.content.title" alt="" class="w-64 pt-3 pb-1">
    <p class="font-medium text-darkBlue/50 text-sm ">{{ index.story.content.description }}</p>
</div>


<div class="max-w-6xl mx-auto pt-20 lg:px-7 grid grid-cols-3 lg:grid-cols-7 md:grid-cols-6 sm:grid-cols-5 pb-48">
...
<script setup>
  ...
  const { data: icons } = await useFetch(`${baseUrl}?starts_with=icons${version+token}`)
